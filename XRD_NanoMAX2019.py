"""
 ...
 This script is not updated. Use script on Boston instead. (20190807)
 
loads data via ptypy and perform xrd analysis from NanoMAX experiment

 Copy from XRD_usingPtypy_NM2017_shifting 
...
* make some tests where you plot a 3d peak and print out the results from the COM
...
p.scans.scan01.data.name = 'NanomaxBraggMarch2019'

#p.scans.scan01.data.datapath = 'H:/NanoMAX_March2019/2019032708/raw/%s/' % sample
p.scans.scan01.data.datapath = 'C:/Users/Sanna/Documents/Beamtime/NanoMAX_March2019/rawdata_selection/%s/' % sample
p.scans.scan01.data.datafile = '%s.h5' % sample
p.scans.scan01.data.detfilepattern = 'scan_%04d_merlin_0000.hdf5'
# not sure if this loads properly

#p.scans.scan01.data.maskfile = 'F:/Susanna/ptypy_scripts/merlin_mask.h5'
...
p.scans.scan01.data.theta_bragg =  7.8

raw_center = (70,140) # for LCBP31_AB
...
#TODO this function is not updated for new data nanomax >2019
# gonphi is the rotation of the rotatation stage on which all motors samx/y/z and samsx/y/z and sample rests on.
# when gonphi is orthoganoal to the incoming beam, it is equal to 180
...
def sort_scans_after_theta():
    ...
    for i in range(0,len(scans)):
        ...
        gonphi_list.append(motorposition_gonphi)
...
dy = 4.7623126945517901e-08  #g.resolution is maybe better. isnt that calculated from pixel sixe, distance to detector energy?
dx = 4.9408562249257234e-08 
...
plt.title('Summed intensity (log). Indices: [40:100,110:170]')
...
plt.title('Summed intensity for all positions for scan %d (log)'%scans_sorted_theta[plot_rotation][1])
...
test_shift_coord = ptypy.core.geometry_bragg.Geo_Bragg.coordinate_shift(g, P.diff.storages.values()[0], input_space='reciprocal',
                         ...
                         layer=max_pos_naive)
# check if the data looks good, no hot pixels
plt.figure()
...
##                import pdb; pdb.set_trace()
#                plt.imshow(np.log10(sum(data_orth_coord.data[0])), cmap='jet')#, extent=[ qy[0], qy[-1], qz[0], qz[-1] ])
...
def plot_XRD_polar():    
    ...
    print 'mean lattice constant is %d' %np.nanmean(a_lattice_exp)
...
plt.figure()
plt.imshow(diff_data[max_pos,len(scans)/2], cmap='jet')
